...
export const inngest = new Inngest({ id: "EMSPRO" });
...
const autoCheckOut = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    if (!attendance?.checkOut) {
      ...
      // After 10 hours, mark the attendance as checked out with status "LATE"
      await step.sleepUntil(
...
const leaveApplicationReminder = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    if (leaveApplication?.status === "PENDING") {
      ...
      // send reminder email to admin to take action on the leave application
      await sendEmail({
...
// Cron: Check attendance at 11:30 AM IST (06: 00 AM UTC) and email absent employees
const attendanceReminderCron = inngest.createFunction(
  ...
  async ({ step }) => {
    ...
    // Step 3: Get employees Ids on approved leave for today
    ...
    // Step 4: Get employees Ids who have checked in today
    const checkedInIds = await step.run("get-checked-in-ids", async () => {
      const attendances = await Attendance.find({
        date: { $get: new Date(today.startUTC), $lte: new Date(today.endUTC) },
      }).lean();
    ...
    if (absentEmployees.length > 0) {
      await step.run("send-reminder-email", async () => {
        const emailPromises = absentEmployees.map((emp) => {
          ...
          sendEmail({
            to: emp.email,
            subject: "Attendance Reminder - Mark Your Attendance",
